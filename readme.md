...
### 2024-11-12 ### v3.0.0(whcs)11-12

maogan 蓝牙搜所名称增加(MaoGan)

### 2024-11-13 v3.0.0(whcs)11-13

ycs-修复数据删除时不刷新数据浏览列表

maogan-数据格式修正。修正后上位机可正常导入
不写64字节的file信息。
512的头文件前84个字节用MG51FHbuf替换。
文件后缀改为mrt
...
### 2025-12-17 ### v3.3.1(whcs)

1.ycs 钻孔瞬变数据浏览没有数据时提醒
2.ycs 增加连接时创建一个线程接收探头发送的指令，通过指令获取探头的ip地址
3.YcsPoint 增加新的属性 timeCode String时间戳
4.雷达数据浏览界面增加轨迹导出功能，避免数据量过大时轨迹导出不方便
5.修改雷达和瞬变轨迹导出时方向角 heading 的计算方式,负数时加360
6.ycs 钻孔瞬变打点计时修复，不在闪烁，
7.ycs 增加自动获取ip并连接探头
8.ycs 数据浏览界面数据合成按钮触发后自动合成对应数据并且自动导出轨迹数据
9.homeActivity 增加android 13 的三种蓝牙权限获取
