...
public class YcsSendBean {

    public byte[] buffer = null;
    public int OutTime = 0;
    int index = 0;

    public YcsSendBean(int data_size, int OutTime)
    {
    ...
    public ReceiveBean SettingOrder(byte code, String FileName, float RAreaX, float RAreaY, float RAreaZ, float TArea, float MSpacing, int SampleCount, float SampleIntervel, float TFreq, int StackCount, int PlusPower, int SampleTimes, float GYOThreshold)
    {
        ReceiveBean order = null;
        index = 0;
        if (FileName.length() > 31)
        {
            ToastUtils.showLong("文件名不能超过31个字母!");
        }
        else
        {
            try
            {
                ...
                short order_length = (short)(1 + 1 + 6 + 32 + 4 * 2 + 8*4);
                byte[] length = ConvertCode.getBytetoCSharp(order_length);
                EncodeInput(length[1]);
                EncodeInput(length[0]);
                ...
                byte[] FloatBuffer = ConvertCode.getBytetoCSharp(RAreaX);
                for (int j = 0; j < 4; j++)
                {
                    sum += EncodeInput(FloatBuffer[j]);
                }

                FloatBuffer = ConvertCode.getBytetoCSharp(RAreaY);
                for (int j = 0; j < 4; j++)
                {
                    sum += EncodeInput(FloatBuffer[j]);
                }

                FloatBuffer = ConvertCode.getBytetoCSharp(RAreaZ);
                for (int j = 0; j < 4; j++)
                {
                    sum += EncodeInput(FloatBuffer[j]);
                }

                FloatBuffer = ConvertCode.getBytetoCSharp(TArea);
                for (int j = 0; j < 4; j++)
                {
                    sum += EncodeInput(FloatBuffer[j]);
                }

                FloatBuffer = ConvertCode.getBytetoCSharp(MSpacing);
                for (int j = 0; j < 4; j++)
                {
                    sum += EncodeInput(FloatBuffer[j]);
                }
                ...
                FloatBuffer = ConvertCode.getBytetoCSharp(SampleIntervel);
                for (int j = 0; j < 4; j++)
                {
                    sum += EncodeInput(FloatBuffer[j]);
                }

                FloatBuffer = ConvertCode.getBytetoCSharp(TFreq);
                for (int j = 0; j < 4; j++)
                {
                    sum += EncodeInput(FloatBuffer[j]);
                }
                ...
                short_byte = ConvertCode.getBytetoCSharp((short)PlusPower);
                for (int i = 0; i < 2; i++)
                {
                    sum += EncodeInput(short_byte[i]);
                }
                short_byte = ConvertCode.getBytetoCSharp((short)SampleTimes);
                for (int i = 0; i < 2; i++)
                {
                    sum += EncodeInput(short_byte[i]);
                }
                FloatBuffer = ConvertCode.getBytetoCSharp(GYOThreshold);
                for (int j = 0; j < 4; j++)
                {
                    sum += EncodeInput(FloatBuffer[j]);
                }
