...
public class YcsSendMethod {
    ...
    public YcsSendMethod(int OutTime, int DataSize, byte code) {
        this.OutTime = OutTime;
        this.DataSize = DataSize;
        this.code = code;
    }

    public YcsDeviceStatus GetdeviceStatus(DatagramSocket sendSocket, String address, int Local_port, int server_prot) {
        YcsDeviceStatus info = null;
        try {
            YcsSendBean order = new YcsSendBean(DataSize, OutTime);
            if (code == 0x00) {
                code = 0x01;
            } else {
                code = 0x00;
            ...
            if (success && cache.DeviceInfo.Data != null && cache.DeviceInfo.Data.length > 0) {
                ...
                tmp4[0] = cache.DeviceInfo.Data[8];
                tmp4[1] = cache.DeviceInfo.Data[9];
                tmp4[2] = cache.DeviceInfo.Data[10];
                tmp4[3] = cache.DeviceInfo.Data[11];

                tmp1[1] = cache.DeviceInfo.Data[12];
                tmp1[0] = cache.DeviceInfo.Data[13];

                info = new YcsDeviceStatus();
                info.status = getint16(tmp1[0], tmp1[1]);
                info.r_quantity = ByteBuffer.wrap(tmp2).order(ByteOrder.LITTLE_ENDIAN).getFloat();
                info.t_quantity = ByteBuffer.wrap(tmp3).order(ByteOrder.LITTLE_ENDIAN).getFloat();
                info.gyro = ByteBuffer.wrap(tmp4).order(ByteOrder.LITTLE_ENDIAN).getFloat();
            }
            if (success) {
                int a = 0;
            }
    ...
    private  final String TAG = "YcsSend";
    ...
    public int GetSetting(DatagramSocket socket,String address, int local_port, String FileName, float RAreaX, float RAreaY, float RAreaZ, float TArea, float MSpacing, int SampleCount, float SampleIntervel, float TFreq, int StackCount, int PlusPower, int SampleTimes, float GYOThreshold)
    {
        int result = -1;
        try
        {
