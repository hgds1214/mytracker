...
import com.zeus.tec.databinding.ActivityDirectionfinderDataViewBinding;
...
import com.zeus.tec.ui.directionfinder.Apater.directionfinderDataListAdapater;
...
public class directionfinderDataViewActivity extends AppCompatActivity {

    public ActivityDirectionfinderDataViewBinding binding;
    private directionfinderDataListAdapater dataListAdapter = new directionfinderDataListAdapater();
    Query<dirctionfinderDrillHoleInfo> query = ObjectBox.get().boxFor(dirctionfinderDrillHoleInfo.class).query()
            .orderDesc(dirctionfinderDrillHoleInfo_.collectionDateTime).build();
    ...
    public void onMergeEvent(MergeEvent event) {
        ...
        List<dirctionfinderDrillHoleInfo> data = dataListAdapter.getData();
        if (data == null) return;

        int size = data.size();
        for (int i = 0; i < size; i++) {
            dirctionfinderDrillHoleInfo info = data.get(i);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityDirectionfinderDataViewBinding.inflate(getLayoutInflater());
        ...
        dataListAdapter.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(@NonNull BaseQuickAdapter<?, ?> adapter, @NonNull View view, int position) {
                ...
                startActivity(directionfinderDataDetailActivity.buildIntent(directionfinderDataViewActivity.this, dataListAdapter.getItem(position)));
            }
        });
    ...
    private void doExport(dirctionfinderDrillHoleInfo info) {
    ...
    private void doShare(dirctionfinderDrillHoleInfo info) {
        if (info.zipPath == null || info.zipPath.isEmpty()) {
            ToastUtils.showLong("数据合成错误，压缩文件不存在");
            return;
        }
    ...
    private void doShowData(dirctionfinderDrillHoleInfo info) {
        if (info == null) return;
        DataCollectActivity.launch(this, info.id, 2);
    }

    private void doContinue(dirctionfinderDrillHoleInfo info) {
        if (info == null) return;
        directionfinderDataCollectActivity.launch(this, info.id, 0);
    }

    private void doMerge(dirctionfinderDrillHoleInfo info) {
        if (info == null) return;
        MesseagWindows.showMessageBox(this, "删除记录", "是否删除当前记录", new DialogCallback() {
            ...
            public void onPositiveButtonClick() {
                ...
                if(result){
                    ObjectBox.get().boxFor(dirctionfinderDrillHoleInfo.class).remove(info.id);
                    dataListAdapter.notifyDataSetChanged();
                    refreshData();
                }
                else {
    ...
    private void loadData(int pageNum) {
        List<dirctionfinderDrillHoleInfo> drillHoleInfos = query.find(pageNum * pageSize, pageSize);
        if (pageNum == 0) {
            dataListAdapter.setNewInstance(new ArrayList<dirctionfinderDrillHoleInfo>());
        }
